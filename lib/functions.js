...
functions.put = function(payload, query, callback) {
	...
	if (paths.length == 2) {
		...
		callback(200, "Bulk update not supported");
		return;
	}
...
functions.patch = function(payload, query, callback) {
	...
	if (paths.length == 2) {
		...
		logger.log_verbose("Bulk update of resources");
		callback(200, "Bulk patch not allowed");
		return;
	}
		...
		if (err) {
			...
			if (doc_type == "posts") {
				...
				if (store.authors.docs[author]) {
					...
					const res_json = {"id": payload.id, "msg":"Failed to find author", "error":"Author Not Found"};
					callback(500, JSON.stringify(res_json));
					return;
				}
			}
